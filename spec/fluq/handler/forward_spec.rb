...
describe Fluq::Handler::Forward do
  ...
  subject do
    described_class.new urls: ["tcp://127.0.0.1:26712", "tcp://127.0.0.1:26713"]
  end

  it { should be_a(Fluq::Handler::Buffered) }
  its(:config) { should == { :pattern=>"*", :flush_interval=>60, :flush_rate=>0, :buffer=>"memory", :urls=>["tcp://127.0.0.1:26712", "tcp://127.0.0.1:26713"] } }
