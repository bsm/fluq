...
describe FluQ::Input::Socket do
  ...
  let!(:input) { described_class.new(reactor.current_actor, bind: "tcp://127.0.0.1:26712") }
  ...
  it 'should require bind option' do
    lambda { described_class.new(reactor.current_actor) }.should raise_error(ArgumentError, /No URL to bind/)
  end

  it 'should bind only to tcp or unix' do
    lambda { described_class.new(reactor.current_actor, bind: 'udp://1234') }.should raise_error(URI::InvalidURIError)
  end
